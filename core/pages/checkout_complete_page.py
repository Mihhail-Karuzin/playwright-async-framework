...
class CheckoutCompletePage(BasePage):
    def __init__(self, page: Page):
        ...
        self.complete_container: Locator = page.locator(".checkout_complete_container")
        self.complete_header: Locator = page.locator(".complete-header")

    async def wait_for_complete_page(self):
        logger.info("Waiting for checkout complete page")
        await self.wait_for_url("/checkout-complete.html")
        await self.wait_for_locator(self.complete_container)

    async def is_order_complete(self) -> bool:
        await self.wait_for_complete_page()
        return await self.complete_header.is_visible()
